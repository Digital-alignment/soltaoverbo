  ...
  return (
    <div className="min-h-screen bg-bgPlataforma text-tintaCarvao pb-24">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 lg:pl-28 pt-4 sm:pt-6 space-y-6">
        ...
        {/* CABEÇALHO DA PÁGINA ALINHADO AO CENTRO DO LAYOUT DA PLATAFORMA */}
        ...
        {/* GRID DE CARDS DAS OFICINAS ALINHADO */}
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 sm:p-5 space-y-3 flex-1 flex flex-col justify-between">
                    ...
                    {/* BOTÕES DE AÇÃO DINÂMICOS COM FONTE EXPANDIDA (DESKTOP & MOBILE) E ÍCONE À DIREITA */}
                    <div className="pt-3 border-t border-papelKraft/30">
                      {isStarted ? (
                        /* CASO B: JÁ COMEOU (CONTINUAR -> TERRACOTA, TEXTO MAIOR FONTE 30-34PX, ÍCONE DIREITA) */
                        <Link
                          to={`/course/${course.id}`}
                          className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-acentoTerracota text-white font-gesto text-[30px] sm:text-[34px] lowercase shadow-md hover:bg-acentoTerracota/90 transition-transform hover:scale-[1.02] cursor-pointer"
                        >
                          <span>continuar</span>
                          <Play className="w-5 h-5 text-white fill-white shrink-0" />
                        </Link>
                      ) : hasAccess ? (
                        /* CASO A: NÃO COMEOU E LIBERADO (COMEÇAR -> VERDE OLIVA, TEXTO BRANCO 28-32PX, ÍCONE DIREITA) */
                        <Link
                          to={`/course/${course.id}`}
                          className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-acentoOliva text-white font-gesto text-[28px] sm:text-[32px] lowercase shadow-md hover:bg-acentoOliva/90 transition-transform hover:scale-[1.02] cursor-pointer"
                        >
                          <span>começar</span>
                          <ArrowRight className="w-5 h-5 text-white shrink-0" />
                        </Link>
                      ) : (
                        /* CASO C: BLOQUEADO PARA UPGRADE (DESBLOQUEAR -> AZUL 28-32PX, TEXTO BRANCO, ÍCONE DIREITA) */
                        <a
                          href={course.stripe_payment_link || '/profile'}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-acentoAzul text-white font-gesto text-[28px] sm:text-[32px] lowercase shadow-md hover:bg-acentoAzul/90 transition-transform hover:scale-[1.02] cursor-pointer"
                        >
