...
export default function HalftoneDotsCanvas() {
  ...
  useEffect(() => {
    ...
    // Color definitions (low -> high): #111111, #525252, #D4D4D4, #FAFAFA
    const colorsData = new Float32Array([
      0.067, 0.067, 0.067, // #111111
      0.322, 0.322, 0.322, // #525252
      0.831, 0.831, 0.831, // #D4D4D4
    ...
    // Uniform Values
    gl.uniform4f(uShapeLoc, 1.26, 0.35, 0.28, 0.00);      // u_shape
    gl.uniform4f(uSurfaceLoc, 1.82, 1.00, 0.00, 1.00);    // u_surface
    gl.uniform4f(uFinishLoc, 0.00, 0.00, 0.000, 0.04);     // u_finish
    gl.uniform4f(uTransformLoc, 1.0, 0.00, 0.00, 0.0);    // u_transform
    gl.uniform4f(uSpaceLoc, 0.00, 0.00, 0.00, 0.00);       // u_space
    gl.uniform4f(uCursorLoc, 0.0, 2.0, 0.65, 0.46);       // u_cursor (presence = 0.0)
    ...
    const render = (now: number) => {
      ...
      // Speed 25/100 -> time = seconds * 0.57 * (25 / 100) = seconds * 0.1425
      const elapsedSeconds = (now - startTime) * 0.001;
      const uTime = elapsedSeconds * 0.57 * 0.25;

      gl.uniform4f(uSceneLoc, canvas.width, canvas.height, uTime, 4.0);
  ...
  return (
    <canvas
      ref={canvasRef}
      className="absolute inset-0 w-full h-full object-cover pointer-events-none opacity-20 mix-blend-overlay z-0"
    />
  );
}
